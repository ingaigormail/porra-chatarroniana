...
if not nombre_usuario:
    ...
    with tab1:
        ...
        with col_stats1:
            ...
            st.info("País(es) que más puntos están sumando en el torneo real.")
            if not df_equipos.empty:
                try:
                    col_pts_eq = [c for c in df_equipos.columns if 'punto' in str(c).lower()][0]
                    col_nom_eq = df_equipos.columns[0] 
                    
                    # Identificar la puntuación máxima
                    max_pts = df_equipos[col_pts_eq].max()
                    
                    # Filtrar todos los equipos que tienen esa puntuación máxima
                    mvps = df_equipos[df_equipos[col_pts_eq] == max_pts]
                    
                    # Mostrar todos los encontrados
                    for _, row in mvps.iterrows():
                        st.success(f"### {con_bandera(row[col_nom_eq])} con {row[col_pts_eq]} puntos")
                except Exception as e:
                    st.warning(f"Error al calcular MVP: {e}")
            else:
